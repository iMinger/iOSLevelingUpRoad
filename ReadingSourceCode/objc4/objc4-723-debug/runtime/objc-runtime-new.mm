...
attachCategories(Class cls, category_list *cats, bool flush_caches)
{
    ...
    int i = cats->count;
    bool fromBundle = NO;
    // 注意： 这里采用了倒序的方式,将所有的分类的方法列表、属性列表、所遵循的协议列表整合成为一个总的mlists，proplists，protolists
    while (i--) {
    ...
    // 这里，将三个新的mlists，proplists，protolists 分别attach 到methods，properties，protocols上（具体是放置到对应数组的前面，具体可看attachLists方法的内部实现）
    // 所以，当有的分类的方法和本类方法同名时，会先找到分类中的方法返回。
    // 同理，当有两个分类都有同一个方法名时，会先找到后编译文件的方法。
