...
- (nullable NSOperation *)queryCacheOperationForKey:(nullable NSString *)key options:(SDImageCacheOptions)options context:(nullable SDWebImageContext *)context done:(nullable SDImageCacheQueryCompletionBlock)doneBlock {
    ...
    //transformer 变压器，促使改革的人
    // 将url对应的key 变换成 在缓存中的key值。
    id<SDImageTransformer> transformer = context[SDWebImageContextImageTransformer];
    if (transformer) {
        // grab the transformed disk image if transformer provided
        NSString *transformerKey = [transformer transformerKey];
        
        // 说明从缓存中查找的key 和 url 是两个不同的字段，从缓存中查找的key对url进行了一些封装
    ...
    // 对查找到的image 进行 操作变换， 最终得到我们需要的image.
    if (image) {
    ...
    /*
     ...
     有两种情况： 1. 内存命中cache,同步到disk磁盘
                2. 内存没有命中cache,磁盘同步到内存
     */
    ...
    // 这里定义一个查询磁盘的block
    // 这个block 里面就封装了disk cache 查询的操作
    void(^queryDiskBlock)(void) =  ^{
        ...
        @autoreleasepool {
            ...
            if (image) {
                ...
                // 对从disk 读取的二进制进行解码，成为UIImage 对象
...
//因为这个分类遵循了这个协议，所以在分类的实现文件中要将协议中必须实现的方法实现。
